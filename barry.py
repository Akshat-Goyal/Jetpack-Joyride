...
class Barry(Person):
    def __init__(self, gridDim):
        self._disp = np.array(
            [['o', '-', 'o'], ['-', '|', '-'], ['/', ' ', '\\']])
        Person.__init__(self, gridDim[0][1] - self._disp.shape[0],
                        gridDim[1][0], self._disp, Fore.CYAN + Back.BLACK)
        self.__fig = [self._disp, np.array(
            [['o', '^', 'o'], ['-', '|', '-'], ['|', ' ', '|']])]
        for i in range(1, 7):
    ...
    def changeDisp(self, obj):
        ...
        if self.__dragonOn:
            obj['bullet'].makeWeapon(
                self._x + int((self._disp.shape[0] - 1) / 2), self._y + self._disp.shape[1], obj)
    ...
    def objCol(self, obj):
        ...
        self.__dragonOn |= obj['dragonBoost'].checkCol(
            self._x, self._y, self._disp, obj)
